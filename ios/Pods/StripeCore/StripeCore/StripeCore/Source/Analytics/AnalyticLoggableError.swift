...
@_spi(STP) extension Error {
    ...
    let errorType: String = {
        if let analyticLoggableError = self as? AnalyticLoggableError {
            return analyticLoggableError.analyticsErrorType
        } else {
            return Self.extractErrorType(from: self)
        }
    }()
    
    let errorCode: String = {
        if let analyticLoggableError = self as? AnalyticLoggableError {
            return analyticLoggableError.analyticsErrorCode
        } else {
            return Self.extractErrorCode(from: self)
        }
    }()
